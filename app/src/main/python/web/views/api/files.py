...
class GetOrUploadView(FileView):
    ...
    @staticmethod
    def _upload_file(article, file_name, stream, user=None, content_type=None):
        if not file_name:
        ...
        new_file = File(name=file_name, media_name=media_name, author=user, article=article)
        ...
        try:
            ...
            with open(new_file.local_media_path, 'wb') as f:
                while True:
                    ...
                    f.write(chunk)
            new_file.size = size
            new_file.mime_type = content_type or 'application/octet-stream'
            articles.add_file_to_article(article, new_file, user=user)
        except:
    ...
    def post(self, request: HttpRequest, article_name):
        ...
        self._upload_file(article, file_name, request, user=request.user, content_type=request.headers.get('content-type'))
